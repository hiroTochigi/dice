...
const default_mode = all_dice_mode_btn[0]
...
const change_mode_display = (all_dice_mode_btn, target) => {

    for (let i=0; i<all_dice_mode_btn.length; i++){
        all_dice_mode_btn[i].style.color = 'black';
        all_dice_mode_btn[i].style.backgroundColor = 'white';
    }
...
const add_event_on_dice_mode_btn = (all_dice_mode_btn) => {
    ...
    for (let i = 0; i<all_dice_mode_btn.length; i++ ){
        all_dice_mode_btn[i].onclick = () => {
            const mode = all_dice_mode_btn[i].innerText
            get_mode(mode)
            control_dice_box(mode)
            change_mode_display(all_dice_mode_btn, all_dice_mode_btn[i])
        }
    }
}
...
change_mode_display(all_dice_mode_btn, default_mode)
control_dice_box("Default")